...
| FR-NLOG-09 | **A credentials email is emptied once it has been delivered** *(owner decision, 2026-08-24, replacing the accepted risk in §2.8)*. `mail` documents are never purged, so a generated tenant password written into an email body stays legible in Firestore forever — for the life of the project, to anyone who reaches the console. When `onMailWrite` sees `delivery.state == 'SUCCESS'` on a document whose `type` is the credentials email (FR-NLOG-03 puts `type` on every write site, which is what makes this identifiable at all), it **replaces the message body with a fixed placeholder**. The body has no further purpose: the email is already delivered, and the notification log keeps `type`, `audience`, recipient and delivery state, which is everything the log displays. Nothing needs to know the password to remove it — the whole body goes. **Two traps.** The function writes to the very document it triggers on, so it must skip an already-redacted document or it loops forever; and `onMailWrite` already fires three or four times per email (§6), so the redaction must be idempotent by construction, like the projection beside it. |
