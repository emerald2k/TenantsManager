...
/**
 * Best-effort delete of a Storage object identified by its download URL.
 * `ref(storage, url)` resolves an https download URL (or a gs:// URI) back to
 * its bucket+path — the same call already proven in ContractUpload/
 * PhotoGallery/PhotoCapture's own delete handlers, reused here as-is.
 *
 ...
 */
export async function deleteAttachmentBestEffort(url) {
  try {
